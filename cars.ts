...
export const CAR_DATABASE: BrandMap = {
  ...
  Opel: {
    ...
    Astra: {
      ...
      "1.6 Turbo 180 HP": { hp: 180, fuel: "benzin" },
...
export function calcStage1(hp: number, fuel: Fuel): { gain: number; after: number; pct: number } {
  ...
  let pct: number;
  if (fuel === "dizel") {
    // Dizel turbo motorlar: %12-22 arası (düşük HP = daha yüksek oran)
    pct = Math.max(12, Math.min(22, 15 + (hp - 90) * 0.03));
  } else if (fuel === "benzin") {
    if (hp < 130) {
      // Atmosferik veya küçük turbo benzin: %3-8 (çok düşük artış)
      pct = Math.max(3, Math.min(8, 4 + (hp - 70) * 0.05));
    } else {
      // Turbo benzin: %10-18
      pct = Math.max(10, Math.min(18, 12 + (hp - 150) * 0.02));
    }
  } else {
    // Hibrit veya diğer
    pct = Math.max(5, Math.min(12, 7 + (hp - 100) * 0.03));
  }

  const gain = Math.round(hp * (pct / 100));
  return { gain, after: hp + gain, pct: Math.round(pct) };
}
